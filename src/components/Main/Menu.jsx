...
const Menu = () => {
    ...
    const dropDown = useTransition(navData.open, {
        ...
        onStart: () => setNavData({ ...navData, ['headerBgHeight']: navData.open === true ? ((navData.headerBgHeight + menuRef.current.clientHeight) + 20) : 199}),
        config: config.stiff
    })
    ...
    return (
        <div>
            <div onClick={() => setNavData({ ...navData, ['open']: !navData.open })} style={{
                    ...
                    item &&
                        <animated.div style={{ 
                            ...
                            <ul ref={menuRef} style={{ listStyle: "none" }}>
                                {
                                    items.activeCat.subCategories.map(({ _id, subCatName, catName }) => (
                                        <LI key={_id} active={subCatName === subcat ? true : false}>
                                            <NavLink to={`/${catName}/${subCatName}`}>
                                                <span>{subCatName}</span>
                                                { 
                                                    subCatName === subcat ? 
                                                        <LineIcon name="checkmark" style={{
                                                            float: "right",
                                                            color: "#F8F7F9",
                                                            fontSize: "1rem"
                                                        }} /> 
