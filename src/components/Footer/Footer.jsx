...
const Footer = () => {
  ...
  const [styles, animation] = useSpring(() => ({ x: 0 }));
  ...
  useEffect(() => {
    if (match?.params.cat !== items.activeCat.catName && match !== null) {
      ...
      items.categories.forEach((cat, i) => {
        if (match.params.cat === cat.catName) ind = i;
      });
      ...
      try {
        const fetchData = async () => {
          const width =
            navRef.current.children[ind].firstElementChild.clientWidth !== 0
              ? navRef.current.children[ind].firstElementChild.clientWidth
              : 125;

          animation.start({ x: width * ind });
          ...
          if (status === 200) {
            setData({
              ...
              activeSubcat: items.categories[ind].subCategories[0],
              contents:
                data.data[items.categories[ind].subCategories[0].subCatName],
            });
    ...
    >
      ...
      >
        {items.categories.map((cat, i) => (
          <NavLink
            key={cat._id}
            to={`/${cat.catName}/${cat.subCategories[0].subCatName}`}
          >
            <div
              style={{
                textAlign: "center",
